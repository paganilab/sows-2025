...
> 💡 Just write `gobj` in your console to inspect the `Giotto` object, everything we had before should be there!
...
**Q1** Which are the top 3 genes for each spatial co-expression cluster?
...
**Q2.** Display the resulting spatial domains you obtained (_hint_: try using the `SpatialDimPlot` function with the `custom_leiden` metadata field)
**Q3.** Calculate the results of the previous clusters with the clustering obtained now, how much is each previous cluster contained in the new ones?
**Q4.** Display cell type proportions across the identified domains using a heatmap (_hint_: see the `pheatmap` package)
```{r, echo=FALSE}
...
**Q5.** Which are the clusters with the highest level of interactions? (_hint_: check out the `cellchatnet$count` object)
**Q6.** Which are the top 3 interactions across `cluster-3` (Tumor) and `cluster-2` (Stroma/Fibroblasts)? (_hint_: use the `subsetCommunication` function from `CellChat`)
...
**Q7.** Plot this in space as well like shown below (_hint_: use the `netVisual_aggregate` function)
```{r, echo=FALSE}
# Spatial plot
par(mfrow=c(1,1))
