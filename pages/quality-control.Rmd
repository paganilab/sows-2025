...
- Compute an additional `meta.data` column named `to_keep` which contains values `Keep` or `Discard`. If a spot passes QC then gets assigned a `Keep` else it is flagged by a `Discard`. (_hint_: check out the `mutate` and `ifelse` function to create the column)
...
- Use the `FindVariableFeatures` function from the `Seurat` package to detect highly-variable genes in the data and keep the top 4,000 genes using the `vst` method
- Check which are the top 10 most variable genes in the data (_hint_: try a combination of the `head` and `VariableFeatures` function)
...
- Draw an elbow plot yourself (_hint_: use the convenient `ElbowPlot` function)
...
- Plot the UMAP displaying the QC information we computed before (`nFeature_RNA`, `nCount_RNA` and `transcriptome.complexity`) using the `FeaturePlot` function
...
markers %>%
    ...
    slice_head(n=5)
```
- Plot the expression of one of the main markers across clusters like shown below (_hint_: use the `VlnPlot` function)
```{r, include=FALSE}
VlnPlot(sp, features = c("KRT19", "COL1A1"))
```
- Plot the expression of one of the main markers on a UMAP embedding like shown below
```{r, include=FALSE}
...
```{r}
# Plot in space
SpatialFeaturePlot(sp, features = ct_to_match[4:6])
```
