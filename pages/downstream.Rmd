...
- Inspect the resulting object, what are the top 3 most _spatially variable_ genes?
...
We can additionally summarize the activity of spatial modules into metascores at single spot level, this is equivalent to calculating a signature score at the single-spot level starting from th e lists of genes composing the modules. We expect to see a spatial patterning in the data.
...
- Which are the top 3 genes for each spatial co-expression cluster?
...
gobj <- createNearestNetwork(gobject = gobj,
                                     dim_reduction_name = "custom_pca",
                                     dimensions_to_use = 1:20, 
                                     k = 5,
                                     name = "custom_NN")
...
- Display the resulting spatial domains you obtained (_hint_: try using the `spatPlot2D` function with the `custom_leiden` metadata field)
- Calculate the results of the previous clusters with the clustering obtained now, how much is each previous cluster contained in the new ones?
...
cellchat <- computeCommunProb(cellchat, type = "truncatedMean", trim = 0.1,
                              ...
                              contact.dependent = TRUE, contact.range = 100, nboot=10, do.fast=TRUE)
# Aggregate results
